...
let batch = JSON.parse(localStorage.getItem('batchHalfMonthPayslips') || '[]');
...
$('addBatchBtn').addEventListener('click', ()=>{
  ...
  localStorage.setItem('batchHalfMonthPayslips', JSON.stringify(batch));
  renderBatch();
  $('payslipForm').reset();
  calculate();
...
$('exportBatchBtn')?.addEventListener('click', ()=>{
  ...
  link.download = 'HalfMonthPayslips.csv';
  link.click();
});
...
$('exportIndividualBtn')?.addEventListener('click', ()=>{
  ...
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = 'Individual_HalfMonthPayslips.csv';
  link.click();
});
...
$('clearAllBtn')?.addEventListener('click', () => {
  ...
  if (confirm('Are you sure to clear all payslips?')) {
    batch = [];
    localStorage.removeItem('batchHalfMonthPayslips');
    renderBatch();
